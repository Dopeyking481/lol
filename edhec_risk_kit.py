...
def get_ffme_returns():
    """
    Load the Fama-French dataset for the rreturns of the Top and Bottom Deciles by MarketCap
    """
...
def semideviation1(r):
    """"
    Returns the semideviation aka negative semideviation of r  
    r must be a Series or Dataframe
    Approximation that is used for data with mean close to zero (as daily returns)
    """
    is_negative = r<0
...
def semideviation2(r):
    """"
    Returns the semideviation aka negative semideviation of r  
    r must be a Series or Dataframe, else raise a TypeError
    MORE ACCURATE
    """
    excess = r-r.mean()
...
def skewness(r):
    ...
    Compute the skewness of the supplied
    Series or DataFrame
    Returns a float or a series
    """
    demeaned_r = r-r.mean()
    # use the population standard deviation sp set dof = 0
...
def kurtosis(r):
    ...
    Compute the kurtosis of the supplied
    Series or DataFrame
    Returns a float or a series
    """
    demeaned_r = r-r.mean()
...
def var_gaussian ( r , level = 5, modified=True):
    ...
    If "modified" is , then the modified Var is returned, 
    using the Corner-Fisher modification
    """
...
def annualized_vol(r, periods_per_year):
    ...
    to the reader :-)
    """
    return r.std()*np.sqrt(periods_per_year**0.5)
...
def plot_ef2 (n_points, er, cov, style=".-"):
    ...
    if er.shape[0] !=2 or er.shape[0] !=2:
        raise ValueError (" plot_ef2 can only plot the 2-asset efficient frontier")
...
def plot_ef (n_points, er, cov):
    """
    Plots the multi- asset efficient frontieer
    """
